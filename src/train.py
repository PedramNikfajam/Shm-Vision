...
# Add project root and src/ to path (script may be run from anywhere).
...
def set_seed(seed: int = 42) -> None:
    """Seed python/numpy/torch RNGs for reproducible training runs."""
    random.seed(seed)
    np.random.seed(seed)
...
def resolve_device(requested) -> str:
    """Resolve a device spec to one that actually works on this machine.

    ``'auto'`` (or None) picks the first CUDA GPU when available, else CPU.
    An explicit GPU id that CUDA cannot satisfy is demoted to CPU with a
    ...
    req = str(requested)
    if req.isdigit() and not cuda_ok:
        logger.warning("CUDA not available; requested device '%s' -> using cpu", req)
        return 'cpu'
    return req


def resolve_workers(requested) -> int:
    """Resolve dataloader workers: ``'auto'``/0-safe default per platform.

    Windows dataloader multiprocessing is fragile with the ultralytics
    training loop, so ``'auto'`` means 0 there; Linux (Kaggle) gets 4.
    """
    if requested in (None, '', 'auto'):
...
def setup_training_args(args: argparse.Namespace, hyperparams: Dict) -> Dict[str, Any]:
    ...
    if hyperparams:
        ...
        training_args['device'] = resolve_device(args.device if args.device else train_cfg.get('device', 'auto'))
        training_args['workers'] = resolve_workers(args.workers if args.workers is not None else train_cfg.get('workers', 'auto'))
        ...
        training_args['cos_lr'] = opt_cfg.get('cos_lr', False)
        ...
        # Absolute path: ultralytics resolves relative project paths against
        # its own runs_dir, which nests output under runs/classify/runs/...
        training_args['project'] = str(PROJECT_ROOT / val_cfg.get('project', 'runs/classify'))
        ...
        training_args['device'] = resolve_device(args.device or 'auto')
        training_args['workers'] = resolve_workers(args.workers if args.workers is not None else 'auto')
        training_args['cache'] = True
        training_args['project'] = str(PROJECT_ROOT / 'runs/classify')
...
def train_model(args: Dict[str, Any]) -> None:
    """Execute YOLOv8 classification training (seeded for reproducibility)."""
    try:
        ...
        for param in ['optimizer', 'lr0', 'lrf', 'momentum', 'weight_decay', 'warmup_epochs', 'cos_lr']:
            if param in args:
